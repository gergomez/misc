...
main(int argn, char *argv[])
{
	...
	while((l = recv(sock_desc_t, buf, BUFSIZE, 0)) > 0) {
	    ...
	    close(sock_desc_t);	/* Ya lo tiene abierto el hijo, el padre debe cerrarla */
	    break;
	}
    }
    return 0;
...
mastermind(int empieza)
{
    ...
    *(hum + M + 1) = 0;
	...
	while(!valido) { 
		...
		lowfrand(p, freq);
	    } else if(!cerca && !vuelta && (rand()%(4*paso) == 0))  { 
		semilla(p); 
	    } else { 
