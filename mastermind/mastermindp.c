    ...
    for (paso = 0; paso < 10; ++paso) {
        ...
        if (paso > 0) {
            ...
            for (flag = 1, mprob = 0, minfo = 2 * M;
                 !intcmp(test, adiv) && flag; nextperm(test)) {
        ...
        for (j = 0; j < M; ++j) {
            ...
            if (!m) {
                for (i = 0; i < M; ++i) {
                    if (dat[i][adiv[j]]) dat[i][adiv[j]] += h + m;
                }
                dat[j][adiv[j]] = 0;
            } else if (!h) {
                dat[i][adiv[j]] += m;
        ...
        putchar('\n');
        for (i = 0; i < M; ++i) {
            for (j = 0; j < N; ++j) printf("%d(%d) ", inf[i][j], dat[i][j]);
            putchar('\n');
        }
        putchar('\n');
    }
